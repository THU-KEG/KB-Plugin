...
class KoPLEngine(object):
	...
	def forward(self, functions, inputs, ignore_error=False, show_details=False):
		programs = []

		def dfs(i, cur_functions, cur_inputs):
			if i == len(functions):
				programs.append((cur_functions, cur_inputs))
				return
			function, input = functions[i], inputs[i]
			if function in ["Find", "FilterConcept"]:
				name = input[0]
				for id in self.kb.name_to_id[name]:
					dfs(i+1, cur_functions+[function], cur_inputs+[[id]])
			else:
				dfs(i+1, cur_functions+[function], cur_inputs+[input])

		dfs(0, [], [])
		results= []
		for cur_functions, cur_inputs in programs:
			res = self.single_forward(cur_functions, cur_inputs, ignore_error, show_details)
			results.append((res, (cur_functions, cur_inputs)))
		return results

	def single_forward(self, program, inputs, 
	...
	def FilterConcept(self, entities, concept_id):
		"""
		找出属于特定概念的所有实体
	...
	def SelectAmong(self, entities, key, op):
		...
		entity_ids = list(set([i for i,v in candidates if v==value])) # 可以有多个最大/最小值
		return entity_ids
