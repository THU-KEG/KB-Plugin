...
from utils.kb_environment import Program, Computer, parse_seq_program, find_entities
...
class ProgramState:
    def __init__(self, tokenizer, current_program, initial_programs, admissible_programs, topic_entity):
        self.tokenizer = tokenizer
        self.initial_programs = defaultdict(list)
        if isinstance(initial_programs, list):
            for program in initial_programs:
                assert isinstance(program.source, set)
...
class KoPLConstrainedDecoder(LlamaForCausalLM):
    ...
    def initial_program_states(self, topic_entities, questions, batch_size, num_beams):
        assert len(topic_entities) == batch_size and len(questions) == batch_size
        program_states = []
        for topic_entity, question in zip(topic_entities, questions):
            initial_programs = self.computer.get_initial_programs(topic_entity, question)
            # for program in initial_programs:
            #     print(program.code)
            #     print()
            # exit()
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        topic_entities, questions = model_kwargs["topic_entities"], model_kwargs["question"]
        batch_size = len(beam_scorer._beam_hyps)
        num_beams = beam_scorer.num_beams
        
        if topic_entities[0] is None:
            qs = []
            for q in questions:
                qs.extend((q + '\n Find <arg>') for i in range(num_beams))
            inputs = self.tokenizer(qs, padding=True, return_tensors="pt").to(input_ids.device)
            splitter_token_ids = [self.tokenizer.encode('<func>', add_special_tokens=False)[0], self.tokenizer.encode('a<func>', add_special_tokens=False)[1]]
            outputs = super().beam_search(
                **inputs,
                beam_scorer=deepcopy(beam_scorer),
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=pad_token_id,
                eos_token_id=splitter_token_ids,
                output_scores=output_scores,
                return_dict_in_generate=False,
                synced_gpus=synced_gpus,
            )
            codes = [output.split('\n')[1].strip()[:-1].strip() for output in self.tokenizer.batch_decode(outputs, skip_special_tokens=True)]
            assert len(codes) == batch_size * num_beams
            topic_entities = []
            for i in range(batch_size):
                topic_entities.append(set())
                for j in range(num_beams):
                    # print(codes[i*num_beams+j])
                    program = parse_seq_program(codes[i*num_beams+j])
                    topic_entities[i].update(x for x in find_entities(program) if x in questions[i])
                for ent in self.spacy_ner(questions[i]).ents:
                    topic_entities[i].add(ent.text)
                topic_entities[i] = list(topic_entities[i])
        ...
        # topic_entities, questions = model_kwargs["topic_entities"], model_kwargs["question"]
        program_states = self.initial_program_states(topic_entities, questions, batch_size, num_beams)
            ...
            for idx, next_token in zip(beam_idx, self.tokenizer.convert_ids_to_tokens(beam_next_tokens)):
                ...
                if next_token not in valid_tokens:
                    ...
                    if len(finished_programs) == 0:
                        ...
                        for program in finished_programs:#[:5]:
                            # print()
                            # print(program.code)
                            # print(program.kopl)
                            admissible_programs.extend(
                                self.computer.get_admissible_programs(
                                    ...
                                    question=questions[idx//num_beams],
                                    expected_hop=model_kwargs["expected_hop"][idx//num_beams] if self._dataset == "metaqa" else None,
                                )
                            )
                        ...
                        if len(admissible_programs) == 0:
                            ...
                            program_state = ProgramState(
                                ...
                                topic_entity=program_state.topic_entity
                            )
                            program_state.move_on_trie([next_token])
        ...
        for i in range(batch_size):
            ...
            for code in output_codes[i*num_return_sequences:(i+1)*num_return_sequences]:
                if self.check_finish(code) and code in code2program_map:
                    program = code2program_map[code][0]
                    print(program.code)
            for code in output_codes[i*num_return_sequences:(i+1)*num_return_sequences]:
                if self.check_finish(code) and code in code2program_map:
                    program = code2program_map[code][0]
                    program.topic_entities = topic_entities[i]
                    try:
                        program.execution = self.computer.execute_program(program)
                    except:
                        finish = 3
                        print("ERROR: ", program.code)
                        continue
                    if len(program.execution) > 0:
                        ...
                        finish = 2
                        print("EMPTY: ", program.code)
            if finish == 1:
                output_programs.append("Exceed Max Length: %s" % output_codes[i*num_return_sequences])
            elif finish == 2:
                output_programs.append(Program(code=output_codes[i*num_return_sequences], topic_entities=topic_entities[i], execution=[]))
            elif finish == 3:
                output_programs.append("API ERROR: %s" % output_codes[i*num_return_sequences])
        assert len(output_programs) == batch_size
        return output_programs
    
    def forward(self, topic_entities=None, question=None, expected_hop=None, **model_inputs):
